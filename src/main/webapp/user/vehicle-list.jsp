            ...
            <c:forEach items="${vehicles}" var="vehicle">
                <div class="vehicle-card" data-company="${vehicle.make}">
                    ...
                    <div class="action-buttons">
                        <a href="user-view-slots?vehicleId=${vehicle.id}" class="btn btn-view">View Slots</a>
                        <c:choose>
                            <c:when test="${vehicle.status.name() != 'SOLD'}">
                                <form action="../vehicleapp/user-buy-vehicle" method="post" style="flex: 1;">
                                    <input type="hidden" name="vehicleId" value="${vehicle.id}">
                                    <button type="submit" class="btn btn-book">Buy Now</button>
                                </form>
                            </c:when>
                            <c:otherwise>
                                <button class="btn" style="background-color: #95a5a6;" disabled>Sold Out</button>
                            </c:otherwise>
                        </c:choose>
                    </div>
                </div>
